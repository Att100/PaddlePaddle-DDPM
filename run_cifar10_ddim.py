...
class _DiffusionModel(DiffusionModel):
    ...
    def sample_image(self, path, postfix, stride=1, n_samples=1, batch_size=1, nrow=1, ncol=1, enable_pbar=False):
        if n_samples == 1:
            img = self.p_sample_loop_ddim((1, 3, 32, 32), stride, eta=0, enable_pbar=enable_pbar)[0][-1]
            ...
            if n_samples <= batch_size:
                imgs = self.p_sample_loop_ddim((n_samples, 3, 32, 32), stride, eta=0, enable_pbar=enable_pbar)[0][-1]
            else:
                ...
                for i in range(n_steps):
                    ...
                    imgs.append(self.p_sample_loop_ddim((batch_size, 3, 32, 32), stride, eta=0, enable_pbar=enable_pbar)[0][-1])
                if remain != 0:
                    print("Step {} ...".format(n_steps+1))
                    imgs.append(self.p_sample_loop_ddim((remain, 3, 32, 32), stride, eta=0, enable_pbar=enable_pbar)[0][-1])
    ...
    def sample_sequence(self, path, postfix, stride=1, n_samples=1, interval=1, enable_pbar=False):
        ...
        _, x0_preds = self.p_sample_loop_ddim(
            (n_samples, 3, 32, 32), stride, eta=0, enable_pbar=enable_pbar)
...
if __name__ == "__main__":
    ...
    options = {
        ...
        'sequence': False,  
        'sample_postfix': "ddim_cifar10_test"
    }
